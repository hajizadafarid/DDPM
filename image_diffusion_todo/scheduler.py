...
class DDPMScheduler(BaseScheduler):
    ...
    def step(self, x_t: torch.Tensor, t: int, eps_theta: torch.Tensor):
        ...
        alphas_cumprod = self._get_teeth(self.alphas_cumprod, t).to(x_t.device)

        mu_tilda = (1/torch.sqrt(alpha_t)) * (x_t - ((1- alpha_t)/torch.sqrt(1- alphas_cumprod)) * eps_theta).to(x_t.device)
        
        sigma_tilda = self._get_teeth(self.sigmas, t).to(x_t.device)
        sample_prev = mu_tilda + sigma_tilda * eps
        sample_prev.to(x_t.device)
        #######################
    ...
    def add_noise(
        ...
        alphas_cumprod = self._get_teeth(self.alphas_cumprod, t).to(x_0.device)
