...
class SimpleNet(nn.Module):
    def __init__(
        ...
        self.num_timesteps = num_timesteps
        self.fc1 = TimeLinear(self.dim_in, self.dim_hids[0], self.num_timesteps)
        self.fc_list = nn.ModuleList([])
        for i in range(len(self.dim_hids)-1):
            self.fc_list.append(TimeLinear(self.dim_hids[i], self.dim_hids[i+1], self.num_timesteps))
        self.fc_last = TimeLinear(self.dim_hids[-1], self.dim_out, self.num_timesteps)
        self.act = nn.ReLU()
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor):
        ...
        x = x.to(device)
        t = t.to(device)
        x = self.act(self.fc1(x, t)).to(x.device)
        for i in range(len(self.dim_hids)-1):
            x = self.act(self.fc_list[i](x, t)).to(x.device)
        x = self.fc_last(x, t).to(x.device)
