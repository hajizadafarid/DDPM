...
class DiffusionModule(nn.Module):
    ...
    def q_sample(self, x0, t, noise=None):
        ...
        # Compute xt.
        alphas_prod_t = extract(self.var_scheduler.alphas_cumprod, t, x0).to(self.device)
        # xt = x0
        # eps_t = torch.rand_like(x0).to(x0.device)
    ...
    def p_sample(self, xt, t):
        ...
        eps_theta = self.network(xt, t).to(self.device)

        alpha_t = extract(self.var_scheduler.alphas, t, xt).to(xt.device)
        var = extract(self.var_scheduler.betas, t, xt).sqrt().to(xt.device)
        zt = torch.randn_like(xt).to(xt.device)
        # x_t_prev = xt
        x_t_prev = (1/alpha_t.sqrt()) * (xt - eps_factor * eps_theta) + var * zt
        x_t_prev = x_t_prev.to(self.device)
    ...
    def p_sample_loop(self, shape):
        ...
        # x0_pred = torch.rand_like(torch.zeros(shape)).to(self.device)
        x0_pred = torch.randn(shape).to(self.device)

        for t in range(self.var_scheduler.num_train_timesteps-1, -1, -1):
            t = torch.tensor([t]).to(self.device)
            x0_pred = self.p_sample(x0_pred, t).to(self.device)
    ...
    def compute_loss(self, x0):
        ...
        eps_theta = self.network(xt, t).to(x0.device)

        loss = F.mse_loss( eps_theta, eps).to(x0.device)
